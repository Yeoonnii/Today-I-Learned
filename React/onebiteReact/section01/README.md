...
# ch17. 배열 (Array)
값을 순차적으로 저장할 수 있는 자료형

- 배열 생성

new 연산자나 대괄호를 사용하여 배열을 생성할 수 있다.
...
배열 생성시 기본 원시타입 데이터, 객체, 새로운 배열, 함수 등 원하는 값을 넣을 수 있다.
...
- 배열 요소에 접근하기
    - `배열명[인덱스]` 로 배열 요소에 접근 할 수 있다.
    - 인덱스는 배열 요소의 순번이다
    - 인덱스는 0부터 시작한다. 즉 배열의 첫번째 요소의 인덱스는 0이다.
    - 존재하지 않는 배열에 접근하려 하는 경우 undefined 를 반환한다.

- 배열 요소 값 수정하기
    - `배열명[인덱스] = 수정할 요소 값` 으로 수정이 가능하다.

- 배열 요소 값 삭제하기
    - `배열명[인덱스] = 삭제할 요소 값` 으로 삭제가 가능하며 삭제된 요소는 empty로 출력된다.

```jsx
delete arr3[2];
console.log(arr3);     
// ['hello', 3.3, empty, false, Array(0), ƒ, ƒ, 'String'] 
// => 배열의 세번째 요소가 삭제되어 empty로 바뀌었다.
```